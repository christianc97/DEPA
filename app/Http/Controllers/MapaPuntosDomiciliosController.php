...
class MapaPuntosDomiciliosController extends Controller
{
    ...
    public function index() {
        ...
        if ($tienePermiso) {
            ...
            $clientes = DB::connection('mu_domicilios')->select('select p.id, p.nombre, empresa_id, p.lat, p.long, p.direccion, p.ciudad, (e.nombre) as empresa, e.mu_ref from puntos p
                left join empresa e on p.empresa_id = e.id 
                where mu_ref is not null group by mu_ref order by ciudad;');
