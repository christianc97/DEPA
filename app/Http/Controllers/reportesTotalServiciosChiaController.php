...
WHERE t.estado IN (5)    
AND t.solicitante IN ( SELECT id FROM tbl_users WHERE empresas_id = 4393) 
AND fecha_inicio BETWEEN :between AND :and
AND t.ciudad_id = 6',["between"=>$fecha_inicio,"and"=>$fecha_fin]);

        Excel::create('reporte total servicios chia desde ' . $fecha_inicio . ' hasta ' . $fecha_fin . '', function($excel)use($total_servicios) {
